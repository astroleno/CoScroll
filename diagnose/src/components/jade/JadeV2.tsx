...
export default function JadeV2({
  ...
  // 背景显隐切换：true 时显示 HDRI 天空，false 时隐藏（保持 envMap 仅用于渲染）
  useEffect(() => {
    ...
    try {
      if (showBackground) {
        ...
        if (scene.background === envBackground) scene.background = null;
      }
    } catch {}
    return () => {
      try {
  ...
  }, [showBackground, envBackground, scene]);
  ...
  const ssrMaterial = useMemo(() => {
    ...
    if (!useDualPassRefraction || !refractionMap) return null;
    try {
      ...
      const mat = createScreenSpaceRefractionMaterial({
        ...
        normalScale,
        useEdgeFade,
        renderResolution: { x: (refractionMap as any).image?.width || res.x, y: (refractionMap as any).image?.height || res.y },
      });
      // Debug：先放大偏移并启用纯红切换开关（默认关闭）
      (mat.uniforms as any).uOffsetBoost.value = 3.0;
  ...
  return (
    <group>
      ...
      <group ref={groupRef} position={[0, 0, 0]} scale={scale}>
        <mesh ref={meshRef} geometry={geometry} material={useDualPassRefraction && ssrMaterial ? ssrMaterial : material} />
      </group>
    </group>
  );
}
